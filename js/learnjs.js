...
var submitGuessBtn = document.querySelector("#submitguess");

var guessList = document.querySelector(".guess_list");
var guessResult = document.querySelector(".guess_result");
var restartBtn = document.querySelector(".restart_btn");

var randomNum;
var jiShu;
startBtn.addEventListener("click", startNewGuess);
restartBtn.addEventListener("click", startNewGuess);

function startNewGuess() {
	if(!startBtn.classList.contains("fade")) {
		startBtn.classList.add("fade");
	}
	if(guessForm.classList.contains("fade")) {
		guessForm.classList.remove("fade");
	}
	if(guessInput.hasAttribute("disabled")) {
		guessInput.removeAttribute("disabled");
	}
	if(submitGuessBtn.hasAttribute("disabled")) {
		submitGuessBtn.removeAttribute("disabled");
	}
	if(!restartBtn.classList.contains("fade")) {
		restartBtn.classList.add("fade");
	}
	guessInput.value = "";
...
submitGuessBtn.addEventListener("click", function() {
	if(jiShu == 1) {
		guessList.innerHTML = "已猜测的数字：";
	}
	var guessValue = document.forms["guess_form"]["guessinput"].value;
	if(!guessValue) {
		guessValue = NaN;
	}
	if(isNaN(guessValue)) {
...
function guessOver() {
	restartBtn.classList.remove("fade");
	guessInput.setAttribute("disabled", "disabled");
	submitGuessBtn.setAttribute("disabled", "disabled");
}

function guessRiOrWr(guessValue) {
	var span = document.createElement("span");
	span.className = "guess_value";
	span.innerHTML = guessValue;
	guessList.appendChild(span);
	guessResult.innerHTML = "";
	var pResult = document.createElement("p");
	if(guessValue == randomNum) {
		pResult.className = "result_right";
		pResult.innerHTML = "恭喜您，猜对啦！";
		guessOver();
	} else {
		if(jiShu >= 10) {
			pResult.className = "result_wrong";
			pResult.innerHTML = "!!!没猜中，游戏结束!";
			guessOver()
		} else {
			if(guessValue > randomNum) {
				pResult.className = "result_wrong";
				pResult.innerHTML = "您猜的数字偏高了！";
			} else if(guessValue < randomNum) {
				pResult.className = "result_wrong";
				pResult.innerHTML = "您猜的数字偏低了！";
			} else {
				pResult.className = "result_wrong";
				var i = Math.trunc(Math.random() * 4);
				pResult.innerHTML = wrongDesc[i];
			}
		}
	}
	guessResult.appendChild(pResult);
}
var wrongDesc = ["哎呀，您的输入有误哦！", "额，输入不对哦", "手抖了吗？重新输入吧！", "哈哈，输入错误！"]